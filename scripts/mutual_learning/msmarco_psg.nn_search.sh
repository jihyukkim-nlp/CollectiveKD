...
device=$1 # e.g., "0"
topk=$2 # e.g., "experiments/colbert.teacher/MSMARCO-psg-train-exp_embs0-exp_beta0/label.py/ranking.tsv"
exp_root=$3 # e.g., "experiments/mutual_learning/finetuned.b36.lr3e6.hn.kd_prf.beta0.5.50k"
step=$4 # e.g., "experiments/mutual_learning/finetuned.b36.lr3e6.hn.kd_prf.beta0.5.50k/MSMARCO-psg/train.py/msmarco.psg.l2/checkpoints/colbert-50000.dnn"
...
# sanity chcek: no previous experiments
previous_labeling=${exp_root}/${experiment}/label.py
[ -d ${previous_labeling} ] && echo "Please remove existing labeling results:\n\n\t rm -r ${previous_labeling}\n" && return
...
n_lines_per_split=200000 # e.g., 50000, 100000, or 200000
queries_split=data/queries.train.reduced.splits.200k #TODO: custom path
...
if [ ${do_split} -eq 1 ];then
    ...
    echo "split \"${queries}\" into multiple queries with ${n_lines_per_split} lines each"
    split -d -l ${n_lines_per_split} ${queries} ${queries_split}/queries.tsv.
fi
...
# exp_root=experiments/finetuned.b36.lr3e6.hn.kd_qe_kmeans_prf_beta0.5 #TODO: custom path
...
[ ! -f "${topk}" ] && echo "${topk} does not exist." && return
topk_split=${topk_dir}/$(basename ${queries_split}) # e.g., ${topk_dir}/queries.train.reduced.splits
...
depth=10 #TODO: custom arg: we need only a few top-ranked documents for feedback documents
...
for i in $(seq -f "%02g" 0 $(expr ${n_splits} - 1));do
    ...
    --root ${exp_root} --experiment ${experiment} --fb_k 0 --beta 0.0 --depth ${depth} \
    --score_by_range
done
...
for timelog in $(ls ${exp_root}/${experiment}/label.py | grep 2021-);do
    rm -v ${exp_root}/${experiment}/label.py/${timelog}/ranking.*
done
