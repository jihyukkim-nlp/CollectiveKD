...
experiment_root=experiments/finetuned.b36.lr3e6.hn #TODO: custom path
index_root=experiments/finetuned.b36.lr3e6.hn/MSMARCO-psg/index.py #TODO: custom path
checkpoint=experiments/finetuned.b36.lr3e6.hn/MSMARCO-psg/train.py/msmarco.psg.l2/checkpoints/colbert-50000.dnn #TODO: custom path
...
# 
ranking=${experiment_root}/${experiment}/label.py/ranking.tsv
ranking_jsonl=${experiment_root}/${experiment}/label.py/ranking.jsonl
...
for i in $(seq -f "%02g" 0 $(expr ${n_splits} - 1));do
    ...
    --root ${experiment_root} --experiment ${experiment} --fb_k 0 --beta 0.0 --depth 1000 \
    --score_by_range
done
echo "sanity check: # of lines in small ranking results && total # of queries"
n_ranking_json=0
for timelog in $(ls ${experiment_root}/${experiment}/label.py | grep 2021-);do
    echo
    echo "timelog: "$timelog
    # 
    small_ranking_json=${experiment_root}/${experiment}/label.py/${timelog}/ranking.jsonl
...
echo -n "" > ${ranking}
for timelog in $(ls ${experiment_root}/${experiment}/label.py | grep 2021-);do
    small_ranking=${experiment_root}/${experiment}/label.py/${timelog}/ranking.tsv
    cat ${small_ranking} >> ${ranking}
    small_ranking_jsonl=${experiment_root}/${experiment}/label.py/${timelog}/ranking.jsonl
...
for timelog in $(ls ${experiment_root}/${experiment}/label.py | grep 2021-);do
    rm -v ${experiment_root}/${experiment}/label.py/${timelog}/ranking.*
done
