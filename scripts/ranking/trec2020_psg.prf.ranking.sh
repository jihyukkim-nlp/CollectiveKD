...
queries=data/trec2020/queries.trec2020.tsv
[ ! -f "${queries}" ] && echo "${queries} does not exist." && return
qrels=data/trec2020/2020qrels-pass.txt
[ ! -f "${qrels}" ] && echo "${qrels} does not exist." && return
collection=data/collection.tsv
[ ! -f "${collection}" ] && echo "${collection} does not exist." && return

# ANN search
topk=${exp_root}/TREC2020-psg/retrieve.py/$(ls ${exp_root}/TREC2020-psg/retrieve.py)/unordered.tsv
if [ ! -f "${topk}" ];then
    ...
    --checkpoint ${checkpoint} --root ${exp_root} --experiment TREC2020-psg
else
...
fb_ranking=${exp_root}/TREC2020-psg/rerank.py/$(ls ${exp_root}/TREC2020-psg/rerank.py)/ranking.tsv
...
ranking=${prf_exp_root}/TREC2020-psg/label.py/$(ls ${prf_exp_root}/TREC2020-psg/label.py/ | grep 20*)/ranking.tsv
if [ ! -f ${ranking} ];then
    ...
    --checkpoint ${checkpoint} --root ${prf_exp_root} --experiment TREC2020-psg
else
...
python -m utility.evaluate.trec_passages --qrels ${qrels} --ranking ${ranking}
echo;echo;echo
