...
# expansion_pt=experiments/pilot_test/trec2019/kmeans.k10.beta1.0.clusters10/label.py/2021-10-15_19.23.17/expansion.pt 

CUDA_VISIBLE_DEVICES=${device} python -m analysis.compute_soft_label_on_topk_passages \
--collection ${collection} --presumed_hn ${presumed_hn} --queries ${queries} --labeled_positives ${labeled_positives} --output analysis/calibration_on_presumed_negatives/trec${trec}/score.colbert.tsv
